...
function RevampedThemeButton({ accentColor, accentMuted, collapsed }) {
  ...
  return (
    ...
    >
      ...
      >
        {isDark ? (
          <Sun className="h-[15px] w-[15px]" strokeWidth={1.85} />
        ) : (
          <Moon className="h-[15px] w-[15px]" strokeWidth={1.85} />
        )}
      </span>
      <span
        className={cn(
...
function SidebarAvatar({ authentikId, assetId, initials, gradient, glow }) {
  ...
  const failed =
    erroredAssetId !== null && erroredAssetId === (assetId ?? null);
  return (
    <div
  ...
  return (
    <li className="relative">
      {active && (
        <span
          ...
          style={{
            ...
            boxShadow: `0 0 9px ${tint(accentColor, 0.85)}`,
          }}
        />
      )}
      <Link
      ...
      >
          ...
          style={
            active
              ? {
                  ...
                  boxShadow: `0 2px 8px -2px ${tint(accentColor, 0.8)}`,
                }
        ...
        {!collapsed && badge > 0 && (
          <span
            className="ml-auto rounded-full px-[7px] py-[1px] text-[10px] font-semibold tabular-nums text-white"
            style={{ background: accentColor }}
          >
            {badge > 99 ? "99+" : badge}
          </span>
...
export default function PortalShell({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(PROFILE_PICTURE_CHANGED_EVENT, load);
  }, []);
  ...
  if (revamped) {
    ...
    const nameParts = {
      preferredName:
        profile?.preferred_name ?? user.preferredName ?? user.preferred_name,
    ...
    const profileRole =
      accent === "red"
        ...
        : formatMemberGroup(resolvedGroup);
          ...
          <div className="flex h-screen overflow-hidden bg-background text-foreground">
            ...
            >
              ...
              >
                ...
                >
                  <p className="whitespace-nowrap text-[13px] font-semibold leading-tight tracking-tight text-foreground">
                    Kappa Theta Pi
                  </p>
                  <p
                    className="whitespace-nowrap text-[10.5px] font-medium uppercase leading-tight tracking-[0.14em]"
                    style={{ color: revamped.light }}
                  >
                    {portalName}
                  </p>
                </div>
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(90deg, ${tint(revamped.light, 0.55)}, transparent)`,
                }}
              />
                ...
                {groups.map((group) => (
                  <div key={group.heading} className="mb-3 last:mb-0">
                    {collapsed ? (
                      <div
                        aria-hidden="true"
                        className="mx-auto mb-2 h-px w-6 bg-border"
                      />
                    ) : (
              ...
              <div className="relative shrink-0 border-t border-border p-2">
                <div
                  className={cn(
                    "mb-1.5 flex items-center transition-all duration-300",
                    collapsed
                      ? "justify-center"
                      : "gap-2.5 rounded-lg border border-border/70 bg-background/50 p-2.5",
                  )}
                >
                  <SidebarAvatar
                  ...
                  <AutoSkeleton loading={!profile}>
                    ...
                    >
                      <p className="truncate text-[12px] font-bold leading-tight text-foreground dark:text-white">
                        {displayName}
                      </p>
                ...
                {isAdminPortal && (
                  <button
                    ...
                    aria-label={
                      adminAccent === "red"
                        ? "Switch admin portal to blue"
                        : "Switch admin portal to red"
                    }
                    className={cn(
                      "group flex w-full items-center rounded-lg py-[7px] text-sm font-medium text-muted-foreground transition-all duration-200 hover:text-foreground",
                      collapsed
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <div
                className={cn(
                  "sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-border bg-card px-4 md:hidden",
                )}
              >
                  ...
                  {flatNav.map(({ href, label, icon: Icon }) => {
                    ...
                    return (
                      <Link
                        ...
                        style={
                          active
                            ? { background: tint(revamped.light, 0.12) }
                            : undefined
                        }
                      >
  ...
  const navLinkClass = (active) =>
    `flex shrink-0 items-center rounded-lg text-sm font-medium transition-colors ${
      collapsed
        ...
        : "w-full gap-2 px-3 py-2 md:gap-3"
    } ${
      active
        ? styles.active
