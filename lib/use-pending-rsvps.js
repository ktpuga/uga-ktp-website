    ...
    getEvents()
      .then((events) => {
        ...
        setCount(events.filter((event) => (
          event.requiresRsvp
          // canRsvp, not just requiresRsvp. Seeing an event and being sent one
          // are different questions: an organiser who targets an event at a
          // group they are not in can see it and is not a recipient. Counting
          // those would badge somebody for an answer the API refuses, giving
          // them a number they cannot clear.
          && event.canRsvp
          && !event.myRsvp
