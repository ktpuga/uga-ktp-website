...
export async function logoutEverywhere() {
  ...
  // so if someone signs out and then goes anywhere near it, they are signed
  // straight back in and signing out becomes impossible. Landing on the
  // homepage rather than /login makes that less immediate, not less real —
  // the header's own login link is one click away. See app/login/page.jsx.
  //
  // A cookie rather than a ?signedout=1 query param on the redirect below,
  // because Authentik matches post_logout_redirect_uri against the provider's
  // configured Redirect URIs — a query string would have to be allow-listed
  // too, and getting that wrong doesn't degrade, it strands the person on
  // auth.ugaktp.com. This needs no Authentik configuration at all.
  //
  ...
  // home is no worse for the SSO session and far less confusing.
  //
  ...
  // Where Authentik sends the browser once it has ended the session. It must
  // match an entry in the provider's **Redirect URIs** list EXACTLY, trailing
  // slash included — Authentik uses that same list for post-logout redirects,
  // and silently ignores a URI that isn't on it, leaving the person parked on
  // auth.ugaktp.com wondering what happened. Change this string and you must
  // change that list in the same breath.
