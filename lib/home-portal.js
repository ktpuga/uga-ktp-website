...
export function homePortal(groups = []) {
  ...
  // Alumni share /member. They had their own portal at /alumni, which was the
  // same pages in amber — every route wrapped the identical component, and
  // ktp-api already treats alumni as a full member (SHARED_ALBUM_GROUPS), so
  // the duplicate bought nothing but a second place for bugs to hide.
  if (groups.includes('chair') || groups.includes('active') || groups.includes('alumni'))
    return '/member';
