...
export { MAX_FILE_SIZE, MIN_IMAGE_HEIGHT, MIN_IMAGE_WIDTH, ACCEPTED_IMAGE_MIME_TYPES };

export function createEmptySlideForm() {
  return {
    file: null,
    title: '',
    subtitle: '',
...
export function convertPointerToFocalPoint(rect, clientX, clientY) {
...
export function getSlideScheduleState(slide, now = Date.now()) {
...
export function formatSlideSchedule(slide) {
...
export function normalizeSlideRecord(record) {
...
export function validateSlideForm(values, { requireFile = false } = {}) {
  const errors = {};
...
export function buildCreateSlideFormData(values) {
...
export function buildUpdateSlidePayload(initialValues, currentValues) {
  const payload = {};
...
export function buildReorderPayload(ids) {
  return { ids: ids.map((id) => String(id)) };
}

export function extractApiErrorMessage(errorBody, fallbackMessage) {
