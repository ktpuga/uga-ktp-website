...
export const NOTIFICATION_TABS = ['announcements', 'calendar', 'meetings', 'polls', 'files', 'interviews', 'committees'];
...
// They stay in NOTIFICATION_TABS above because the API still returns them under
...
//
// `committees` is here for a sharper reason than meetings. Its unit of "seen"
// is the COMMITTEE, not the tab: the number is the sum of per-committee counts,
// each cleared by opening that committee. Zeroing it on arrival would blank the
// roll-up while every per-committee marker underneath it stayed lit, so the
// sidebar and the page would openly disagree. It drains as committees are read.
const CLEARS_ON_ACTION_NOT_VIEW = new Set(['meetings', 'committees']);
