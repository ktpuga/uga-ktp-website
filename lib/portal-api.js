...
// Rush-facing: published schedules only, with seats remaining and which slot is
// yours. Never includes other candidates' names — the API doesn't select them,
...
// Booked interviews only, event-shaped, for the calendar. Same split as
// getCalendarMeetings: the calendar rule lives server-side in findForCalendar
// so the portal and the ICS feed can't drift.
...
// May fail with a 409 the caller is expected to show: someone took the last
// seat, or you already have a time booked. Both are normal on signup night, not
// exceptional — see bookSlot in interviewsController.
...
export async function createInterviewSchedule({ title, description, location }) {
  const accessToken = await requireAccessToken();
...
// Also the publish switch. Only the keys present are changed, so passing just
// { published: true } leaves the rest alone.
export async function updateInterviewSchedule(id, changes) {
  const accessToken = await requireAccessToken();
  return apiPatch(`/interviews/schedules/${id}`, accessToken, changes);
}

// 409s with code 'has_bookings' unless force is true — deleting a schedule
// people have interview times in has no undo.
export async function deleteInterviewSchedule(id, { force = false } = {}) {
  const accessToken = await requireAccessToken();
  return apiDelete(`/interviews/schedules/${id}${force ? '?force=true' : ''}`, accessToken);
}

export async function createInterviewSlot(scheduleId, { startsAt, endsAt, location, capacity, interviewerId }) {
  const accessToken = await requireAccessToken();
...
export async function updateInterviewSlot(id, { startsAt, endsAt, location, capacity, interviewerId }) {
  const accessToken = await requireAccessToken();
  return apiPatch(`/interviews/slots/${id}`, accessToken, {
    ...(startsAt !== undefined ? { starts_at: startsAt } : {}),
    ...(endsAt !== undefined ? { ends_at: endsAt } : {}),
    ...(location !== undefined ? { location } : {}),
    ...(capacity !== undefined ? { capacity } : {}),
    ...(interviewerId !== undefined ? { interviewer_id: interviewerId } : {}),
  });
}

// Same force rule as deleteInterviewSchedule.
export async function deleteInterviewSlot(id, { force = false } = {}) {
  const accessToken = await requireAccessToken();
  return apiDelete(`/interviews/slots/${id}${force ? '?force=true' : ''}`, accessToken);
}
