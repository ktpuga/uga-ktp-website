...
export async function createEvent(payload) {
  ...
  const postEventPayload = async (eventPayload) => {
    ...
    try {
      ...
      return {
        body: { error: 'Fetch failed - contact infrastructure committee' },
  ...
  if (firstResult.response.ok) {
    return { ok: true, event: firstResult.body };
  }
  ...
  if ([400, 422].includes(firstResult.response.status)) {
    ...
    if (secondResult.response.ok) {
      return { ok: true, event: secondResult.body };
    }

    const secondError = secondResult.body ?? {};
    return {
      ok: false,
      error: secondError.message ?? secondError.error ?? secondError.detail ?? 'Failed to create event',
    };
  }
  ...
  return {
    ok: false,
    error: firstError.message ?? firstError.error ?? firstError.detail ?? 'Failed to create event',
  };
}
...
export async function createAnnouncement({ title, body, audience }) {
  const accessToken = await requireAccessToken();
  return apiPost('/announcements', accessToken, { title, body, audience: audience || null });
}
...
export async function sendMessage(recipientId, body) {
  const accessToken = await requireAccessToken();
  return apiPost('/messages', accessToken, { recipient_id: recipientId, body });
}
...
export async function sendGroupChatMessage(id, body) {
  const accessToken = await requireAccessToken();
  return apiPost(`/group-chats/${id}/messages`, accessToken, { body });
}
