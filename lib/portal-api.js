...
// fail with "that name is taken" (409), which the caller has to show next to
// the username field rather than as a whole-form failure.
export async function updateUsername(username) {
  const accessToken = await requireAccessToken();
  return apiPut('/users/me/username', accessToken, { username });
}
