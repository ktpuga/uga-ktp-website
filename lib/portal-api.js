...
export async function getMessageableMembers() {
  ...
  // The other half of the same rule, mirroring ktp-api's RUSH_DM_GROUPS: only
  // leadership may message a rushee. /members deliberately returns rushees to
  // every member group (they help run rush and need to see who's coming
  // through), so without this the New Message picker would keep offering a
  // rushee to a pledge or alumnus and the send would just 403.
  //
  // The API is still the boundary — this only stops the app from offering a
  // conversation it already knows it will refuse to carry.
  if (groups.some((g) => LEADERSHIP_GROUPS.includes(g))) return members;
  return members.filter((m) => m.memberGroup !== 'rush');
}
