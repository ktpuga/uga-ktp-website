...
// Turns a throwing apiRequest call into { data } / { error }.
//
// This file is 'use server', so every export is a Server Action, and React
// REPLACES a thrown error's message in production with the #441 digest
// ("An unexpected response was received from the server"). apiRequest throws on
// every non-ok response, so any action that lets a throw escape converts the
// API's own sentence into a number the member cannot act on. That is not
// hypothetical: adding a group to a group chat reported exactly #441, and the
// real 4xx message never reached the screen.
//
// "It's eboard-only so a throw is fine" is the reasoning that produced that
// bug. Eboard has to read failures too.
//
// The redirect error is rethrown rather than reported: apiRequest calls
// Next's redirect() on a 401, which signals by throwing, and swallowing it
// would strand a signed-out member on a page that silently does nothing.
async function resultOf(promise, fallback) {
  try {
...
async function sendEventPayload(method, path, payload, failureMessage) {
  ...
  const eventPayload = {
    ...
    // ⚠ MUST be sent on every update, not only when it is true. PUT /events/:id
    // is a whole-row write (`requires_rsvp = $10`), so omitting this key here
    // silently turns RSVP OFF for any event edited from the portal — including
    // an edit that only changed the title. Same trap requiresAttendance above
    // already lives with. Independent of attendance in both directions.
    requiresRsvp: Boolean(payload.requiresRsvp),
...
// RSVP — "are you coming?", asked before the event. A completely separate
// concept from attendance below: attendance records who actually turned up.
// The two flags are independent, so never infer one from the other.
//
// Returns { error } rather than throwing, because a rejection here is
// something a member has to read: a 403 means the event was not sent to them
// and a 409 means it is already over, and both must land beside the button
// rather than as React #441. See updateProfile for the same rule.
export async function setEventRsvp(eventId, status) {
  const accessToken = await requireAccessToken();
  try {
    return await apiPut(`/events/${eventId}/rsvp`, accessToken, { status });
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Could not save your RSVP' };
  }
}

// Eboard or the event's creator only — a plain member reads their own answer
// from `myRsvp` on the event itself and never calls this.
export async function getEventRsvps(eventId) {
  const accessToken = await requireAccessToken();
  return apiGet(`/events/${eventId}/rsvps`, accessToken);
}
...
export async function createRushAnnouncement({ title, body, links }) {
  const accessToken = await requireAccessToken();
  return apiPost('/rush-announcements', accessToken, { title, body, links: links ?? [] });
}

export async function updateRushAnnouncement(id, { title, body, links }) {
  const accessToken = await requireAccessToken();
  return apiPut(`/rush-announcements/${id}`, accessToken, { title, body, links: links ?? [] });
}

// Same split as the main board: multipart only when there are files.
export async function createRushAnnouncementWithMedia(formData) {
  return sendMultipart('/rush-announcements', 'POST', formData, 'Failed to post announcement');
}

export async function updateRushAnnouncementWithMedia(id, formData) {
  return sendMultipart(`/rush-announcements/${id}`, 'PUT', formData, 'Failed to update announcement');
}

export async function deleteRushAnnouncementMedia(mediaId) {
  const accessToken = await requireAccessToken();
  return apiDelete(`/rush-announcements/media/${mediaId}`, accessToken);
}
...
//
// Swallows its own errors and returns []: this decides whether a NAV ENTRY
// exists, so a backend hiccup must hide the tab rather than break the sidebar
// on every page of the portal. An empty list is
...
// Interview notes
//
// The most sensitive thing this file touches: what an interviewer wrote about a
// candidate. Every function here returns { error } rather than throwing, and
// that is not the usual boilerplate reason — a refusal here is specifically a
// PERMISSION message ("only eboard and the interviewers running this slot"),
// and React #441 would replace it with a generic render error, leaving someone
// staring at a panel that failed for no stated reason.
//
// Addressed by BOOKING id, which is what every rushee chip already carries.
// Storage on the API side keys on the candidate and the round, so these survive
// a candidate rescheduling — see the ktp-api migration.

// -> { access, notes } | { error }
//
// `access` is 'all' or 'own', and the caller MUST render the difference: 'own'
// means the reader withdrew from the slot and is seeing only what they wrote
// themselves, with other people's notes deliberately withheld. Treating that
// list as complete shows "1 note" when there are three.
//
// It comes from the API rather than being derived from `slot.i_am_interviewing`
// on purpose. A client-side copy of a server rule is free to drift the day the
// rule changes, and this is the rule where drifting is worst.
export async function getCandidateNotes(bookingId) {
  const accessToken = await requireAccessToken();

  let res;
  try {
    res = await fetch(`${API_URL}/interviews/bookings/${bookingId}/notes`, {
      cache: 'no-store',
      headers: { Authorization: `Bearer ${accessToken}` },
    });
  } catch {
  ...
  const body = await res.json().catch(() => ({}));
  if (!res.ok) {
    return { error: body.message ?? 'Could not load notes for this candidate' };
  }
  return { access: body.access ?? 'all', notes: Array.isArray(body.notes) ? body.notes : [] };
}

// Upsert YOUR OWN note. There is one per person per candidate per round, so this
// is a PUT and needs no create-versus-update branch — saving twice edits.
//
// `field` is passed through for the same reason the profile forms use it: the
// only 400 this can produce is the length cap, and that message belongs beside
// the textarea rather than in a banner at the top of the page.
export async function saveCandidateNote(bookingId, body) {
  const accessToken = await requireAccessToken();
  ...
  if (!res.ok) {
    return { error: payload.message ?? 'Could not save that note', field: payload.field ?? null };
  }
  return { ok: true, note: payload };
}

// Yours, or anyone's if you run interviews. Note that a 404 here is not only
// "already gone" — the API deliberately answers 404 rather than 403 for a note
// the caller cannot see, so that note ids cannot be walked to count how many
// evaluations exist. The wording below has to be true of both.
export async function deleteCandidateNote(noteId) {
  return interviewDelete(
    `/interviews/notes/${noteId}`,
    'Could not delete that note',
  );
}

// Eboard's decision-night view: every note in a round, grouped by candidate.
// Eboard and chairs only — the API 403s everyone else, including members who
// are running slots in the round.
export async function getRoundNotes(scheduleId) {
  const accessToken = await requireAccessToken();

  let res;
  try {
    res = await fetch(`${API_URL}/interviews/schedules/${scheduleId}/notes`, {
      cache: 'no-store',
      headers: { Authorization: `Bearer ${accessToken}` },
    });
  } catch {
  ...
  const body = await res.json().catch(() => ({}));
  if (!res.ok) {
    return { error: body.message ?? 'Could not load notes for this round' };
  }
  return { candidates: Array.isArray(body) ? body : [] };
}
...
// Eboard-typed traits shown on the member's directory card and on the PUBLIC
// roster. Its own call rather than fields in the profile payload, mirroring the
// API: `traits` is not a profile field there, so the member's own save cannot
// set them however the body is shaped.
//
// Returns { error } rather than throwing. Every rejection here is one an eboard
// member has to read and act on ("Hometown value is required", "You can add up
// to 6 traits"), and a thrown Server Action error becomes React's #441 digest in
// production. See TODO trap #8.
export async function adminUpdateTraits(authentikId, traits) {
  try {
    const accessToken = await requireAccessToken();
    const result = await apiPut(`/admin/users/${authentikId}/traits`, accessToken, { traits });
    return { traits: result?.traits ?? [] };
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Failed to save traits' };
  }
}
...
async function adminMutate(path, method, accessToken, body) {
  ...
  const payload = await res.json().catch(() => ({}));
  if (!res.ok) {
    // `field` is only sent by the routes that validate one field at a time
    // (currently the profile and username saves). Undefined everywhere else,
    // and every caller treats it as optional, so this stays safe for the other
    // admin mutations that share this helper.
    return { error: payload.message ?? 'Request failed', field: payload.field };
  }
  return payload;
}
...
// `group` narrows to one member group. Used by surfaces that want one slice of
// the chapter; the directory itself wants everyone and uses the function below.
...
// Everyone the directory has a tab for: chapter members plus rushees.
//
// Two requests because they genuinely are two sets. `/members` deliberately
// omits rushees — they come back only when asked for by group (see ktp-api's
// memberModel.findAll) — and the directory now shows them as a tab rather than
// a separate page, so it needs both halves. Paired here rather than in the
// component so the browser still makes one round trip.
//
// The rush half is allowed to fail. The API already answers [] rather than 403
// for a caller who may not see rushees, so a rejection here means a real
// backend problem, and that must cost the Rushees tab rather than the whole
// directory. A redirect still propagates: swallowing it renders the literal
// string "NEXT_REDIRECT" instead of navigating.
export async function getMemberDirectoryWithRushees() {
  const accessToken = await requireAccessToken();
  const [members, rushees] = await Promise.all([
    apiGet('/members', accessToken),
    apiGet('/members?group=rush', accessToken).catch((err) => {
      if (isRedirectError(err)) throw err;
      return [];
    }),
  ]);
  return [...formatMemberDirectory(members), ...formatMemberDirectory(rushees)];
}
...
//
// Returns { error } instead of going through apiPut, which throws. Same reason
// as updateUsername below: a thrown Server Action error has its message
// replaced in production by React's generic #441, and every 400 this endpoint
// returns is one the member has to read and act on — a phone number with too
// few digits, a graduation that is not a semester and a year, a date of birth
// in the future, a name that is only spaces, an address that is not UGA's.
// Throwing would turn all of those into "an error occurred".
export async function updateProfile(payload) {
  const accessToken = await requireAccessToken();

  let res;
  try {
    res = await fetch(`${API_URL}/users/me/profile`, {
      method: 'PUT',
      cache: 'no-store',
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  const body = await res.json().catch(() => ({}));
  if (!res.ok) {
    // `field` is the API naming which input it rejected, so the form can put
    // the message beside that input instead of in the banner at the top. It is
    // absent on anything that is not a single-field rejection (a 500, a fetch
    // failure, an older API), and the form falls back to the banner for those,
    // so this must stay optional rather than becoming a required contract.
    return { error: body.message ?? 'Failed to save profile. Please try again.', field: body.field };
  }
  return body;
}
...
// Reorganising the library. Both take null for "the top level", and both are
// open to cabinet as well as eboard — moving is tidying, not a visibility
// change. The API re-checks that the caller can SEE both ends, so a 404 here
// means the destination is invisible to them, not that it is missing.
export async function moveDocument(id, folderId) {
  const accessToken = await requireAccessToken();
  return apiPatch(`/documents/${id}/folder`, accessToken, { folder_id: folderId ?? null });
}

// The API rejects a move into the folder's own subtree with a 400 whose
// message is written for a person, so it is shown as-is rather than replaced.
export async function moveDocumentFolder(id, parentId) {
  const accessToken = await requireAccessToken();
  return apiPatch(`/documents/folders/${id}/parent`, accessToken, { parent_id: parentId ?? null });
}

// Renaming, also open to cabinet: it destroys nothing and changes nobody's
// access. A document's label is `filename` whether it is an upload or a link,
// and renaming an upload never touches the file on disk.
export async function renameDocument(id, filename) {
  const accessToken = await requireAccessToken();
  return apiPatch(`/documents/${id}`, accessToken, { filename });
}

export async function renameDocumentFolder(id, name) {
  const accessToken = await requireAccessToken();
  return apiPatch(`/documents/folders/${id}`, accessToken, { name });
}
...
// `collection_id` is passed only when the caller means to move the photo. The
// API leaves an absent key alone and treats an explicit null as "unfile", so
// spreading it in unconditionally would unfile every photo whose metadata was
// edited without touching its gallery.
export async function updateHomepagePhoto(id, { title, caption, collection_id }) {
  const accessToken = await requireAccessToken();
  return apiPut(`/homepage-photos/${id}`, accessToken, {
    title,
    caption,
...
/* ── Gallery collections ─────────────────────────────────────────────────
 * Named groups over the public homepage gallery. Nothing to do with the iOS
 * slideshow (`ios-homepage-slideshow` below), which is a separate table for
 * the authenticated app.
 * --------------------------------------------------------------------- */

// Public, like getHomepagePhotos(): the landing page and /gallery are both
// rendered for anonymous visitors, so no access token.
//
// `featured` asks for only the collections eboard has promoted, and the API
// caps that list. The cap lives there rather than here because the reason for
// it is server-side: the media endpoint serves ORIGINAL assets with no
// thumbnail variant, so an unbounded landing page gets slower with every
// gallery added.
export async function getGalleryCollections({ featured = false } = {}) {
  if (!API_URL) {
    throw new Error('No API URL found. Contact the infrastructure committee.');
  }
  ...
  let res;
  try {
    res = await fetch(`${API_URL}/homepage-photos/collections${featured ? '?featured=true' : ''}`, {
      cache: 'no-store',
    });
  } catch {
    throw new Error('Fetch failed. Contact the infrastructure committee.');
...
// Eboard's list: photo counts instead of the photos themselves.
export async function getGalleryCollectionsForManagement() {
  const accessToken = await requireAccessToken();
  return apiGet('/homepage-photos/collections/manage', accessToken);
}

export async function createGalleryCollection(payload) {
  const accessToken = await requireAccessToken();
  return apiPost('/homepage-photos/collections', accessToken, payload);
}

export async function updateGalleryCollection(id, payload) {
  const accessToken = await requireAccessToken();
  return apiPut(`/homepage-photos/collections/${id}`, accessToken, payload);
}

// Returns `{ error, code, photo_count }` rather than throwing, because the
// 409 here is a warning a person has to read and act on: deleting a collection
// takes its photos with it. A thrown Server Action error becomes React #441 in
// production and the count never reaches them. Same shape and same reason as
// deleteInterviewSlot.
export async function deleteGalleryCollection(id, { force = false } = {}) {
  const accessToken = await requireAccessToken();

  let res;
  try {
    res = await fetch(`${API_URL}/homepage-photos/collections/${id}${force ? '?force=true' : ''}`, {
      method: 'DELETE',
      cache: 'no-store',
      headers: { Authorization: `Bearer ${accessToken}` },
    });
  ...
  const body = await res.json().catch(() => ({}));
  return {
    error: body.message ?? 'Failed to delete the collection',
    code: body.code,
    photo_count: body.photo_count,
  };
}

export async function reorderGalleryCollections(ids) {
  const accessToken = await requireAccessToken();
  return apiPut('/homepage-photos/collections/reorder', accessToken, { ids });
}

// Sends a FormData body to the API and normalizes the failure shape.
//
// Not exported: this file is 'use server', so every export must be an async
// function a client may call, and a helper reachable from the browser is a
// helper someone points at an arbitrary path.
//
// Kept as a direct fetch rather than routed through apiRequest() for the reason
// every upload here is — that helper sets Content-Type: application/json
// whenever a body is present, which strips the multipart boundary and the API
// rejects the body. Never set Content-Type by hand on a FormData request.
async function sendMultipart(path, method, formData, fallback) {
  const accessToken = await requireAccessToken();
  ...
  const body = await res.json().catch(() => ({}));
  // { error } rather than a throw: a thrown Server Action error is redacted to
  // a generic digest in production, and "that file is over the 100 MB limit" is
  // exactly the message the person needs to see.
  if (!res.ok) return { error: body.message ?? fallback };
  return body;
}
...
export async function createAnnouncement({ title, body, audience, committeeId, sendEmail, links }) {
  const accessToken = await requireAccessToken();
  return apiPost('/announcements', accessToken, {
    title,
    body,
    // Always sent, even empty: the API reads an absent key as "no links", and
    // an edit that silently kept old links would be the surprising one.
    links: links ?? [],
...
export async function updateAnnouncement(id, { title, body, audience, committeeId, links }) {
  const accessToken = await requireAccessToken();
...
// The same two endpoints, when the post carries photos or videos.
//
// A separate function rather than a flag, because the wire format genuinely
// differs: multipart sends every field as a string, so `audience` and `links`
// go as JSON text and the API parses them back. The caller builds the FormData
// so the File objects never have to round-trip through a JSON boundary.
//
// Uploading is part of THIS request and not a follow-up call: the API fires
// push and email the moment it responds, so a two-step upload notifies the
// chapter about a post whose photos have not arrived yet.
export async function createAnnouncementWithMedia(formData) {
  return sendMultipart('/announcements', 'POST', formData, 'Failed to post announcement');
}

export async function updateAnnouncementWithMedia(id, formData) {
  return sendMultipart(`/announcements/${id}`, 'PUT', formData, 'Failed to update announcement');
}

// Removing one photo from a post. New files APPEND on update, so this is the
// only way media comes off.
export async function deleteAnnouncementMedia(mediaId) {
  const accessToken = await requireAccessToken();
  return apiDelete(`/announcements/media/${mediaId}`, accessToken);
}
...
// Per-committee "what is new for me": `[{ committee_id, new_count, pending_count }]`.
//
// Its own endpoint rather than extra columns on getCommittees(), deliberately —
// both announcements pages call getCommittees() purely to turn a committee_id
// into a name, and they should not pay for four subqueries per committee.
//
// `new_count` and `pending_count` are kept apart all the way to the UI: one is
// news to read, the other an approval queue to work. Only the nav badge adds
// them.
export async function getCommitteeActivity() {
  const accessToken = await requireAccessToken();
  return apiGet('/committees/activity', accessToken);
}

// Marks ONE committee read. Called when a member opens a committee's detail
// view — not when they open the Committees page, which is the whole point of
// the per-committee cursor.
export async function markCommitteeSeen(id) {
  const accessToken = await requireAccessToken();
  return apiPost(`/committees/${id}/seen`, accessToken, {});
}
...
//
// `joinCommittee` above no longer joins; it asks. Committee membership grants
// the committee's group chat, everything restricted to that committee, and
// eligibility to run interviews, so it goes through a chair or eboard now.
//
// These four return `{ error }` rather than throwing. Every refusal here is one
// a person has to read and act on — "that request is no longer pending" when
// another chair got there first, or the 403 when a chair opens a committee that
// isn't theirs — and a thrown Server Action error is replaced by React's #441
// digest in production. See trap #8.

export async function getCommitteeJoinRequests(id) {
  try {
    const accessToken = await requireAccessToken();
    return { requests: await apiGet(`/committees/${id}/requests`, accessToken) };
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Failed to load join requests' };
  }
}

export async function approveCommitteeJoinRequest(id, userId) {
  try {
    const accessToken = await requireAccessToken();
    await apiPost(`/committees/${id}/requests/${userId}/approve`, accessToken, {});
    return { ok: true };
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Failed to approve that request' };
  }
}

// Also how somebody withdraws their own request — the API decides which it is
// from whether `userId` is the caller, so there is one route for both.
export async function denyCommitteeJoinRequest(id, userId) {
  try {
    const accessToken = await requireAccessToken();
    await apiDelete(`/committees/${id}/requests/${userId}`, accessToken);
    return { ok: true };
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Failed to update that request' };
  }
}

export async function removeCommitteeMember(id, userId) {
  try {
    const accessToken = await requireAccessToken();
    await apiDelete(`/committees/${id}/members/${userId}`, accessToken);
    return { ok: true };
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Failed to remove that member' };
  }
}
...
// Eboard designating a committee as one whose members may read the rushee
// interest form data. Eboard-only at the API, NOT chair -- a chair who could
// set this on their own committee would be granting themselves the GPAs it
// gates.
export async function setCommitteeRushDataAccess(id, enabled) {
  const accessToken = await requireAccessToken();
  return apiPut(`/committees/${id}/rush-data-access`, accessToken, {
    can_view_rush_data: enabled,
  });
}

// ─── Rushee interest form data ───
//
// Not under /admin, and that is the point of the whole endpoint: the pledge
// committee reaches it too, and a committee is a Postgres row rather than an
// Authentik group, so no route-level group gate can express the rule. The API
// decides in its controller.

// The table itself, and the source of the CSV export.
export async function getRushInterestData() {
  const accessToken = await requireAccessToken();
  const data = await apiGet('/rush-data', accessToken);
  return Array.isArray(data) ? data : [];
}

// "May I see it?", for deciding whether to render the nav entry at all.
//
// Swallows its own errors and answers false, exactly like
// getInterviewerSchedules: this is called on every portal page load to build
// the sidebar, and a backend hiccup should hide one tab rather than break
// navigation everywhere. A tab that appears and then 403s is worse than no tab.
export async function getRushDataAccess() {
  try {
    const accessToken = await requireAccessToken();
    const data = await apiGet('/rush-data/access', accessToken);
...
// Whether the member appears on the PUBLIC roster at /members-list.
//
// Its own endpoint rather than a field on updateProfile, because that route is
// a whole-row upsert and this column is NOT NULL — a client that didn't send
// the key would flip somebody's answer. See ktp-api's routes/users.js.
//
// Returns { error } rather than throwing. This is a setting about what the open
// internet can see, so a failed write is exactly the kind of message a person
// has to read, and a thrown Server Action error becomes React's #441 digest in
// production (TODO trap #8).
export async function setRosterVisibility(visible) {
  try {
    const accessToken = await requireAccessToken();
    await apiPut('/users/me/roster-visibility', accessToken, { visible });
    return { ok: true };
  } catch (err) {
    if (isRedirectError(err)) throw err;
    return { error: err.message ?? 'Could not change your roster visibility' };
  }
}
...
// { chat } / { error }, matching createMemberGroupChat below. It used to throw,
// which meant "Pick at least one group, committee or member for this chat." and
// every other 400 from this route reached eboard as React #441.
export async function createGroupChat({ name, memberIds, audience, committeeIds }) {
  const accessToken = await requireAccessToken();
  const result = await resultOf(
    apiPost('/group-chats', accessToken, {
      name,
  ...
  return result.error ? { error: result.error } : { chat: result.data };
}

// A chat a MEMBER made, as opposed to an official chapter one. Its own export
// rather than a flag on createGroupChat because it posts to a different
// endpoint and accepts no audience or committee list at all: a member's chat is
// exactly the people they picked, and nothing that auto-follows a role.
//
// Returns { chat } / { error } instead of throwing. This file is 'use server',
// so every export here is a Server Action, and a thrown message is redacted to
// React error #441 in production. The failures on this path are ones a person
// has to read ("Rushees can't be added to member group chats"), so they cannot
// be allowed to throw.
export async function createMemberGroupChat({ name, memberIds }) {
  const accessToken = await requireAccessToken();
  let response;
  try {
    response = await fetch(`${API_URL}/group-chats/member`, {
      method: 'POST',
      cache: 'no-store',
  ...
  if (!response.ok) {
    return { error: body?.message ?? 'Failed to create group chat' };
  }
  return { chat: body };
}

// Rename a chat. { chat } / { error } for the same #441 reason: the failure a
// person actually hits here is the 403 from administering someone else's chat.
export async function renameGroupChat(id, name) {
  const accessToken = await requireAccessToken();
  let response;
  try {
    response = await fetch(`${API_URL}/group-chats/${id}`, {
      method: 'PATCH',
      cache: 'no-store',
      headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
  ...
  const body = await response.json().catch(() => ({}));
  if (!response.ok) {
    return { error: body?.message ?? 'Failed to rename this group chat' };
  }
  return { chat: body };
}

// Remove yourself from a chat. { ok } / { error } for the same #441 reason as
// above: every failure here is a 409 whose message IS the explanation, such as
// being in the chat through a committee rather than as an individual.
export async function leaveGroupChat(id) {
  const accessToken = await requireAccessToken();
  let response;
  try {
    response = await fetch(`${API_URL}/group-chats/${id}/leave`, {
      method: 'DELETE',
      cache: 'no-store',
      headers: { Authorization: `Bearer ${accessToken}` },
    });
  ...
  const body = await response.json().catch(() => ({}));
  return { error: body?.message ?? 'Failed to leave this group chat' };
}
...
// left alone.
// { chat } / { error }. THIS is the call that reported React #441 when eboard
// tried to add a group to a chat: the route answers 409 for a member-created or
// the built-in eboard chat and 403 for one you may not administer, and every
// one of those sentences was being replaced by the digest.
export async function setGroupChatAudience(id, { audience, committeeIds }) {
  const accessToken = await requireAccessToken();
  const result = await resultOf(
    apiPatch(`/group-chats/${id}/audience`, accessToken, {
      audience: audience ?? [],
      committee_ids: committeeIds ?? [],
    }),
    'Could not update who is in this chat.',
  );
  return result.error ? { error: result.error } : { chat: result.data };
}
