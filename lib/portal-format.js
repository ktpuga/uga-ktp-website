...
export function toDirectoryMember(raw) {
  return {
    ...
    // Carried purely so the card's avatar can cache-bust on it. Dropping it
    // here is what made the directory the slowest surface to notice a new
    // photo: the id was in the API response and thrown away one line before
    // the component that needed it. See lib/avatar.js.
    profilePictureAssetId: raw.profile_picture_asset_id ?? raw.profilePictureAssetId ?? null,
    doingNow: raw.doing_now ?? raw.doingNow ?? null,
    // Portal only. The API's public roster queries deliberately do not select
    // this column, so on /members-list it is absent and resolves to null —
    // which is the correct outcome, not an oversight to be "fixed" by adding
    // it to those SELECTs. See the migration for why.
    pronouns: raw.pronouns ?? null,
    // Defaulted to an array rather than passed through, because the card maps
    // over it. The API's column is NOT NULL DEFAULT '[]' so this should always
    // be a list already — but a row written before the migration, or a shape
    // from somewhere else entirely, would be `undefined.map` at render time.
    links: Array.isArray(raw.links) ? raw.links : [],
    // Eboard-typed label/value pairs. Defaulted for the same reason as links:
    // the card maps over it.
...
export function safeExternalHref(value) {
  ...
  let url;
  try {
    url = new URL(/^[a-z][a-z0-9+.-]*:/i.test(raw) ? raw : `https://${raw.replace(/^\/+/, '')}`);
  } catch {
    return null;
  }
  if (url.protocol !== 'https:' && url.protocol !== 'http:') return null;
...
// `graduation_date` is TEXT holding a SEMESTER, not a date — "Spring 2027".
// It is already the string we want to show, so the job here is mostly to not
// mangle it.
//
// This used to call `new Date(value)` on it unconditionally, and V8's lenient
// legacy parser silently ignores the word it doesn't recognise and keeps the
// year: `new Date('Fall 2027')` is **1 January 2027**, not an Invalid Date. So
// the NaN guard below never fired, and every semester rendered as "Jan <year>"
// — right year, wrong month, for everybody at once.
export function formatGraduationDate(value) {
  if (!value) return null;
  const text = String(value).trim();
  if (!text) return null;

  // Legacy rows written before the column held a semester are ISO-shaped.
  // Month and year are read straight out of the string and rebuilt in LOCAL
  // time rather than passed through `new Date(iso)`, which parses as UTC
  // midnight and lands on the previous month for a day-01 value west of UTC.
  // That is the same day-shift trap as `dateOnly` in the API.
  ...
  // "Spring 2027" and anything else a person typed: show it as stored.
  return text;
}
...
// A trait is a plain string as of ktp-api migration 1788200000000, but it was a
// { label, value } pair before that. This coerces either into the one string a
// pill renders.
//
// It exists because React THROWS on an object child ("Objects are not valid as
// a React child"), so the un-migrated shape doesn't render badly, it takes the
// whole page down — and the page in question is /members-list, which is public
// and unauthenticated. Without this, deploying the site before running the
// migration is an outage rather than a cosmetic lag.
//
// The join matches the migration's own conversion, so a member sees the same
// text either side of it.
export function traitText(trait) {
  if (typeof trait === 'string') return trait.trim();
  if (trait && typeof trait === 'object') {
    return [trait.label, trait.value].map((p) => String(p ?? '').trim()).filter(Boolean).join(': ');
  }
