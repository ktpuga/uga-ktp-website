...
// Releases a slot before deliberately starting a journey that ENDS at that
// entry point, so the cooldown can't mistake a real arrival for a loop.
//
// The cooldown cannot tell "Authentik bounced me straight back" from "I just
// spent thirty seconds creating an account and here I am legitimately" — both
// are simply a second hit on /auth/start. Getting that wrong swallows a
// successful signup: enrollment finishes, `next=` lands on /auth/start, the
// slot is still held from an earlier attempt in this tab, and the brand-new
// rushee is dropped on /login instead of the profile builder.
//
// The caller has the context the guard lacks. /rush/how-it-works knows the
// person is deliberately leaving for Authentik's enrollment flow, so it says
// so, and the arrival that follows is treated as the first one it really is.
export function clearAutoSignInSlot(name) {
  try {
    sessionStorage.removeItem(`ktp_sso_auto_at_${name}`);
  } catch {
    // Ignored — same reasoning as takeAutoSignInSlot below.
  }
}
