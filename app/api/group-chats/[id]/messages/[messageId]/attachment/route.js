...
// Same reasoning as the photos/documents media proxies — browsers can't
// attach an Authorization header to an <img src> or <a href> request, so
// this runs server-side, attaches the Bearer token, and streams ktp-api's
// response straight through (Range header forwarded too, for images).
export async function GET(request, { params }) {
  ...
  let apiRes;
  try {
    apiRes = await fetch(`${API_URL}/group-chats/${id}/messages/${messageId}/attachment`, { headers: fetchHeaders });
  } catch {
